...
def train_epoch(model, data_loader, optimizer, loss_fun, feed_tgt = False):
    ...
    for xx, yy in data_loader:
        ...
        loss = torch.zeros_like(yy)#loss = 0
        for i, q in enumerate([0.25, 0.5, 0.75]):
            e = yy_pred[:,:,:,i] - yy
            loss += torch.max(q * e, (q - 1) * e)
            
        loss = loss.mean()#loss = loss_fun(yy_pred, yy)
    ...
    return preds, trues, np.mean(mse) #np.round(np.sqrt(np.mean(mse)), 5)
...
def eval_epoch(model, data_loader, loss_fun, concat_input = False):
    ...
    with torch.no_grad():
        for xx, yy in data_loader:
            xx, yy = xx.to(device), yy.to(device)
            yy_pred = model(xx, yy.shape[1])
            loss = torch.zeros_like(yy)       
            for i, q in enumerate([0.25, 0.5, 0.75]):
                e = yy_pred[:,:,:,i] - yy
                loss += torch.max(q * e, (q - 1) * e)
            loss = loss.mean()
            mse.append(loss.item())#loss = loss_fun(yy_pred, yy)
            if concat_input:
